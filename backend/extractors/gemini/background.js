...
async function getAllConversations() {
  const result        = await chrome.storage.local.get(STORAGE_KEY);
  const conversations = result[STORAGE_KEY] || {};
  return Object.values(conversations).sort((a, b) => new Date(b.saved_at) - new Date(a.saved_at));
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'GET_BACKEND_URL') {
    chrome.storage.local.get(BACKEND_URL_KEY).then(r => {
      sendResponse({ url: r[BACKEND_URL_KEY] || DEFAULT_BACKEND });
    });
    return true;
  }

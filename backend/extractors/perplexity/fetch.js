/**
 * Brain Shadow — Perplexity AI Content Script
 *
 * Target: https://www.perplexity.ai
 * Conversation URLs: /search/[slug-hash]  e.g. /search/what-is-ml-AbCdEfGh
 * Architecture matches ChatGPT extractor.
 */

const PLATFORM = 'perplexity';

// Perplexity search/conversation URLs
const CONV_URL_RE = /\/(?:search|page|c)\/([a-zA-Z0-9_\-]{4,})/;
...
function waitForStreamingToFinish() {
  return new Promise((resolve) => {
    ...
    console.log('[Brain Shadow] Waiting for Perplexity to finish…');
  ...
  try {
    if (!overlayEl) {
      ...
      overlayEl.style.cssText = [
        ...
        'background:linear-gradient(135deg,#20b2aa,#0e6b66)',
    ...
    overlayEl.innerHTML = `
      <div style="margin-bottom:8px"><strong>🔍 Brain Shadow — Perplexity</strong></div>
      ...
      <div style="width:260px;height:4px;background:rgba(255,255,255,.3);border-radius:2px;overflow:hidden">
        <div style="width:${pct}%;height:100%;background:#7ae8e8;border-radius:2px"></div>
      </div>
...
function getConversationContainer() {
  const anchor = document.querySelector('[class*="prose"],[class*="answer"],[class*="message"],[class*="response"]');
  if (anchor) {
...
function extractPerplexityId(url) {
  try {
...
function scrapeConversation() {
  ...
  const title = document.title?.replace(' - Perplexity', '').replace(' | Perplexity', '').trim() || external_id;

  console.log(`[Brain Shadow] Perplexity scraped: ${messages.length} messages — "${title}"`);
...
function waitForChatAndObserve() {
  ...
  console.log('[Brain Shadow] Observer attached (Perplexity)');
...
function extractRecents() {
  ...
  console.log(`[Brain Shadow] Perplexity <a> tags: ${allLinks.length}`);
  ...
  for (const link of allLinks) {
    ...
    const title =
      ...
      link.querySelector('p,span,div,h1,h2,h3')?.innerText?.trim() ||
      link.innerText?.trim() ||
      `Perplexity search ${threads.length + 1}`;
  ...
  if (threads.length === 0) {
    ...
    const SKIP = /\/(login|signup|settings|help|about|privacy|terms|logout|new|upgrade|discover|hub)\b/i;
    for (const link of allLinks) {
      ...
      if (seen.has(canonical)) continue; seen.add(canonical);
      const title = link.getAttribute('aria-label')?.trim() || link.innerText?.trim() || `Search ${threads.length + 1}`;
      threads.push({ url: canonical, title: title.slice(0, 120) });
    }
  ...
  console.log(`[Brain Shadow] Perplexity conversations: ${threads.length}`);
  return threads;
}
...
async function scrollSidebarToLoadAll() {
  ...
  if (!sidebar) {
    for (const sel of ['nav','aside','[role="navigation"]','[class*="sidebar"]','[class*="Sidebar"]','[class*="history"]','[class*="library"]']) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.type === 'PING') {
    const count = document.querySelectorAll('[class*="answer"],[class*="prose"],[class*="response"],[class*="message"]').length;

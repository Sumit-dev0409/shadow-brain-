// Brain Shadow — Grok Popup Script

const PLATFORM_URL = 'grok.com';
const BASE_URL     = 'https://grok.com';
const EXTRA_WAIT   = 2500;
const EXPORT_NAME  = 'brain_shadow_grok';
...
async function loadStats() {
  ...
  ['grok','chatgpt','claude','gemini','deepseek','perplexity'].forEach(p => {
...
document.getElementById('btnBulkImport').addEventListener('click', async () => {
  ...
  // Accept both grok.com and x.com/i/grok
  if (!tab.url?.includes(PLATFORM_URL) && !tab.url?.includes('x.com/i/grok')) {
    showToast('Open grok.com first', 'error'); return;
  }
  isBulkRunning = true;

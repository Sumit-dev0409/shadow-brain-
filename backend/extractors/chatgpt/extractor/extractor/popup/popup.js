...
document.getElementById('btnBulkImport').addEventListener('click', async () => {
  ...
  if (!isSupported) {
    showToast('Open ChatGPT or Claude first', 'error');
    return;
  }
    ...
    // Wait helper: poll until content script responds and messages are stable
      ...
      (async () => {
        ...
        for (let i = 0; i < chatUrls.length; i++) {
          const url = chatUrls[i];
          const title = url.split('/c/')[1]?.substring(0, 20) || `Chat ${i + 1}`;
          ...
          try {
            ...
            console.log('[Brain Shadow] Navigated to:', url);

            // Wait for page to be fully ready and messages stable
            await waitForPageReady();
            console.log('[Brain Shadow] Page ready');
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
              try {
                ...
                // IMPORTANT:
                // only continue to next chat AFTER save finishes
                if (
            ...
            savedCount++;
            console.log('[Brain Shadow] Processed count:', savedCount);
          } catch (e) {

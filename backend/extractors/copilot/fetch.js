 ...
 *
 * Conversation URLs: /c/[uuid] or /?conversationId=...
 * All hrefs logged to console on every GET_SIDEBAR_CHATS scan.
 */

const PLATFORM    = 'copilot';
// Covers /c/[id], /chats/[id], /chat/[id], /thread/[id], /conversations/[id]
...
function waitForStreamingToFinish() {
  return new Promise((resolve) => {
    ...
    console.log('[Brain Shadow] Waiting for Copilot to finish…');
...
function showOverlay(data) {
  try {
    ...
    overlayEl.innerHTML = `
      <div style="margin-bottom:8px"><strong>🪟 Brain Shadow — Copilot</strong></div>
...
function scrapeConversation() {
  const USER_SEL = [
    ...
    '[class*="humanMessage"]', '[class*="UserBubble"]',
    '[aria-label*="You said" i]', '[data-role="user"]',
  ];
  const ASST_SEL = [
    '[data-testid*="assistant-message"]', '[data-testid*="copilot-message"]',
  ...
  const external_id = extractCopilotId(location.href);
  const title = document.title?.replace(' - Microsoft Copilot', '').replace(' | Copilot', '').trim() || external_id;

  console.log(`[Brain Shadow] Copilot scraped: ${messages.length} messages — "${title}"`);
...
function extractRecents() {
  ...
  const allHrefs = [...new Set(allLinks.map(a => a.getAttribute('href')))];
  console.log(`[Brain Shadow] Copilot <a> tags: ${allLinks.length}`);
  console.log('[Brain Shadow] ALL hrefs (check these!):\n' + allHrefs.slice(0, 50).join('\n'));
  ...
  // Pass 2 — broad UUID fallback (any same-origin link with an 8+ char ID segment)
  if (threads.length === 0) {
    console.warn('[Brain Shadow] Regex matched 0 — trying broad UUID scan');
    const SKIP = /\/(login|signup|settings|help|about|privacy|terms|logout|new|upgrade|billing|account|profile|home)\b/i;
    for (const link of allLinks) {
    ...
    if (threads.length > 0) console.log(`[Brain Shadow] Broad scan found ${threads.length} — update CONV_URL_RE to match these URLs`);
  }

  if (threads.length === 0) console.warn('[Brain Shadow] 0 conversations — paste DEBUG_PASTE_IN_CONSOLE.js for diagnosis');
  console.log(`[Brain Shadow] Copilot conversations: ${threads.length}`);
  return threads;
}

// Brain Shadow — Microsoft Copilot Popup Script

const PLATFORM_URL = 'copilot.microsoft.com';
const BASE_URL     = 'https://copilot.microsoft.com';
const EXTRA_WAIT   = 2000;
const EXPORT_NAME  = 'brain_shadow_mscopilot';
...
async function loadStats() {
  ...
  ['mscopilot','chatgpt','claude','gemini','deepseek','blackbox'].forEach(p => {
    const badge = document.getElementById(`badge-${p}`);
    if (!badge) return;
    if (platforms[p]) { badge.classList.add('active'); badge.innerHTML = `<span class="dot"></span>${p === 'mscopilot' ? 'MS Copilot' : p.charAt(0).toUpperCase() + p.slice(1)} (${platforms[p]})`; }

/**
 * Brain Shadow — Blackbox AI Content Script v2
 *
...
const PLATFORM    = 'blackbox';
const CONV_URL_RE = /\/chat\/([a-zA-Z0-9_\-]{4,})/;
...
const ASST_SEL = [
  ...
  '[class*="botMessage"]', '[class*="BlackboxResponse"]',
  '[class*="modelResponse"]',
];
...
const STREAMING_INDICATORS = [
  ...
  '[class*="StreamingIndicator"]', '[class*="generating"]',
  '[class*="loading"]',
];
  ...
  try {
    if (!overlayEl) {
      ...
      overlayEl.style.cssText = [
        ...
        'border:1px solid #1cda7a','max-width:300px',
      ].join(';');
      document.body.appendChild(overlayEl);
    }
    const pct = data.percentage || 0;
    overlayEl.innerHTML = `
      <div style="margin-bottom:8px"><strong style="color:#1cda7a">■ Brain Shadow — Blackbox</strong></div>
      ...
      <div style="width:260px;height:4px;background:rgba(255,255,255,.15);border-radius:2px;overflow:hidden">
        <div style="width:${pct}%;height:100%;background:#1cda7a;border-radius:2px"></div>
      </div>
...
function scrollToLoadAllMessages() {
  return new Promise((resolve) => {
    ...
    const tick = () => {
      pos += step;
      c.scrollTop = pos;
      setTimeout(() => { pos < c.scrollHeight ? tick() : resolve(); }, 500);
    };
    setTimeout(tick, 400);
...
function deduplicateConsecutiveRoles(messages) {
  ...
  for (let i = 1; i < messages.length; i++) {
    ...
    if (messages[i].role === prev.role) {
      if (messages[i].content.length > prev.content.length) out[out.length - 1] = messages[i];
...
function extractBlackboxId(url) {
  try {
    const parts = new URL(url).pathname.split('/').filter(Boolean);
    const idx   = parts.findIndex(p => p === 'chat');
...
function scrapeConversation() {
  ...
  if (!userEls.length && !asstEls.length) {
    for (const sel of ['[class*="chatArea"]','[class*="messageList"]','main']) {
  ...
  const external_id = extractBlackboxId(location.href);
  const title       = document.title?.replace(' - Blackbox AI', '').replace(' | Blackbox AI', '').trim() || external_id;

  console.log(`[Brain Shadow] Blackbox scraped: ${messages.length} messages — "${title}"`);
...
function waitForChatAndObserve() {
  ...
  console.log('[Brain Shadow] Observer attached (Blackbox)');
...
function extractRecents() {
  ...
  console.log(`[Brain Shadow] Blackbox <a> tags: ${allLinks.length}`);
  ...
  for (const link of allLinks) {
    ...
    const title =
      ...
      link.querySelector('p,span,div')?.innerText?.trim() ||
      link.innerText?.trim() ||
      `Blackbox chat ${threads.length + 1}`;
  ...
  console.log(`[Brain Shadow] Blackbox conversations: ${threads.length}`);
  return threads;
}
...
async function scrollSidebarToLoadAll() {
  ...
  if (!sidebar) {
    for (const sel of ['nav','aside','[role="navigation"]','[class*="sidebar"]','[class*="history"]']) {

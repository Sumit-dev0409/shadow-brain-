...
const GLOW_COLORS = [
  ...
  "rgba(55,65,81,",     // ChatGPT — dark grey
...
export function ObsidianGraph({ searchKeyword, onNodeClick }: ObsidianGraphProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const nodesRef = useRef<Node[]>([]);
  const frameRef = useRef<number>(0);
  const timeRef = useRef<number>(0);
  const searchRef = useRef<string>("");
  const nodePositionsRef = useRef<Map<number, { x: number; y: number; radius: number; highlighted: boolean }>>(new Map());
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        const node = nodes[i];
        node.highlighted = kw.length > 1 && node.category === (node.id % 4) && Math.random() < 0.002
          ? true
          : node.highlighted;

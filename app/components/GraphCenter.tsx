...
export function GraphCenter({ searchKeyword, onNodeSelect, sessions = [] }: GraphCenterProps) {
  ...
  // Build the set of node IDs to highlight — one per matching session
  const highlightedNodeIds = useMemo(() => {
    const ids = new Set<number>();
    matchingSessions.forEach((s) => ids.add(sessionToNodeId(s.id)));
    return ids;
  }, [matchingSessions]);

  // When a node is clicked, find which session(s) map to it
  const sessionsForNode = useMemo(() => {
    if (!selectedNode) return [];
    return matchingSessions.filter((s) => sessionToNodeId(s.id) === selectedNode.nodeId);
          ...
          {kw.length > 1 && (
            ...
            >
              ✦ {matchingSessions.length > 0
                ? `${matchingSessions.length} node${matchingSessions.length > 1 ? "s" : ""} highlighted for "${searchKeyword}" — click a node`
